...
function addImage(ind) {
	...
	for (i = start; i < end; i++) {
		...
		if($(main_block_name+'#dropped-files > .image').length <= maxFiles) { 
			$(main_block_name+'#dropped-files').append('<div id="img-'+i+'"  class="image" style="background: url('+dataArray[i].value+'); background-size: cover;" iid="'+i+'">'+
			                            // '<a href="javascript:void(0);" id="main-'+dataArray[i].id_pic+'" iid="'+dataArray[i].id_pic+'" class="main-button">Сделать миниатюрой</a>'+
	...
	if (curent_main == 0) {
		$(main_block_name+'#main-'+0).addClass('main-button-active').html('Миниатюра');
	    $(main_block_name+'.main_img').val(0);
	}else{
		$(main_block_name+'#main-'+curent_main).addClass('main-button-active').html('Миниатюра');
	}
...
$(document).on('click',main_block_name+'.main-button',function (){
  $(main_block_name+'.main-button').removeClass('main-button-active').html('Сделать миниатюрой');

  $(this).addClass('main-button-active').html('Миниатюра');
